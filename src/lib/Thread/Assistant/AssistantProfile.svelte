<script lang="ts">
  ...
  import { AssistantSchema } from './Schema.ts';
  import { Assistants, activeAssistants, AssistantProfiles, showSettings, selectedAssistantId } from './Store.ts';
  import { getAssistants, getAssistant } from './getAssistant.ts';
  ...
  <aside class="bg-white dark:bg-blue-800 rounded-md my-4 py-2 h-16">
    ...
    <div class="grid grid-cols-6 md:grid-cols-8 lg:grid-cols-12 justify-between items-center">
      <!-- Settings -->
      <button class="col-span-1 hidden md:block material-icons base_icon items-center w-2 h-2 mr-1 text-gray-400" on:click={toggleSettings}>
        settings
      </button>
      <!-- アシスタントのアイコン -->
      ...
      <div class="col-span-5 col-end-9 hidden lg:block text-center">
        <h2 class="text-2xl font-bold text-blue-900 dark:text-blue-100">{selectedAssistant ? selectedAssistant.name : 'Assistant'}</h2>
        <p class="text-blue-500 dark:text-blue-400 overflow-hidden line-clamp-2">{selectedAssistant ? selectedAssistant.description : 'description'}</p>
      </div>
      <!-- アシスタントのドロップダウンリスト -->
      <div class="col-span-4 col-end-auto relative z-50" 
      on:click={() => dropdownOpen = !dropdownOpen} 
      on:keydown={(e) => { if (e.key === 'Enter') dropdownOpen = !dropdownOpen; }} 
      role="button" 
      tabindex="0">
        <button
          type="button"
        ...
        {#if dropdownOpen}
          <div class="absolute w-full bg-white pt-2">
            ...
            <button type="button" class="grid grid-cols-7 p-2 cursor-pointer rounded border-b-4 border-white hover:border-blue-100 w-full"
  ...
  <!-- アシスタントの詳細設定表示 -->
  <details open class="bg-blue-100 dark:bg-blue-800 p-4 shadow-md static {$showSettings ? 'rounded-t-md' : 'rounded-md'}">
    ...
    <div class="grid grid-cols-12 gap-2 pt-2">
      {#each Object.keys(AssistantSchema.shape) as key}
        <div class="col-span-3 text-clip break-all text-blue-500 dark:text-blue-400 font-bold">{key}</div>
