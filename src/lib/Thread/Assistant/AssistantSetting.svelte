<script lang="ts">
  import { z } from 'zod'
  import { AssistantSettingSchema } from './Schema'
  import { showSettings, selectedAssistantId } from './Store'
  import { createAssistant, updateAssistant, deleteAssistant } from './getAssistant'
    ...
    <div class="p-4 bg-white shadow rounded">
        ...
        {#each Object.keys(AssistantSettingSchema.shape) as key}
          ...
          {#if AssistantSettingSchema.shape[key] instanceof z.ZodBoolean}
            <label class="inline-flex items-center mt-2">
              <input
                type="checkbox"
                bind:checked={assistantSettings[key]}
          ...
          {:else}
            <input
              type="text"
              bind:value={assistantSettings[key]}
              placeholder={key}
              class="mt-2 p-2 border rounded"
            />
          {/if}
      ...
      <div class="mt-4 space-x-2 relative">
        {#if SettingId}
          <button
            class="btn-default"
            title={$selectedAssistantId}
            on:click={() =>
              $selectedAssistantId &&
              updateAssistant(backendUrl, $selectedAssistantId, assistantSettings)}>Update</button
          >
        {:else}
          <button
            class="btn-default"
            title="New Assistant"
            on:click={() => createAssistant(backendUrl, assistantSettings)}>Create</button
          >
        {/if}
        ...
        {#if SettingId}
          <button
            ...
            title={$selectedAssistantId}
            on:click={() =>
              $selectedAssistantId && deleteAssistant(backendUrl, $selectedAssistantId)}
            >Delete</button
          >
        {/if}
      </div>
