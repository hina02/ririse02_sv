...
import type { AssistantSchemaType, AssistantSettingSchemaType } from './Schema.ts';
import { Assistants, activeAssistants, AssistantProfiles } from './Store.ts';
...
export async function updateAssistants(backendUrl: string) {
    let assistants: AssistantSchemaType[] = await fetch(`${backendUrl}/openai_api/get_assistants`).then(r => r.json());
...
export async function getAssistant(backendUrl: string, assistant_id: string): Promise<any> {
    ...
    const currentAssistantProfile = currentAssistantProfiles ? currentAssistantProfiles[assistant_id] : undefined;

    // ストアにAssistantProfilesがない場合、バックエンドから指定Assistantに関連するAssistantProfilesを取得
    if (!currentAssistantProfile) {
        const assistant = await fetch(`${backendUrl}/openai_api/get_assistant/${assistant_id}`).then(r => r.json());
...
// 新しいアシスタントを作成する関数
export async function createAssistant(backendUrl: string, data: AssistantSettingSchemaType): Promise<any> {
...
// 既存のアシスタントを更新する関数
export async function updateAssistant(backendUrl: string, assistant_id: string, data: AssistantSettingSchemaType): Promise<any> {
    console.log(`data: ${JSON.stringify(data, null, 2)}`)
