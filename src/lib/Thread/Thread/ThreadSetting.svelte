<script lang="ts">
  import { z } from 'zod';
  import { ThreadSchema, ThreadSettingSchema } from './Schema.ts';
  import { showSettings, activeThread } from './Store.ts';
  import { createThread, deleteThread } from './getThread.ts';
  ...
  <details class="bg-blue-100 dark:bg-blue-800 p-4 shadow-md rounded-t-md z-20">
    ...
    <div class="grid grid-cols-12 gap-2 pt-2">
      {#each Object.keys(ThreadSchema.shape) as key}
        <div class="col-span-3 text-ellipsis overflow-hidden text-blue-500 dark:text-blue-400 font-bold">{key}</div>
    ...
    <div class="p-4 bg-white shadow rounded">
      ...
      <div class="flex flex-col">
        ...
        {#if key === 'tags'}
          ...
          {:else}
            <input type="text" bind:value={threadSettings[key]} placeholder={key} class="mt-2 p-2 border rounded"/>
          {/if}
        {/each}
      </div>
      <div class="mt-4 space-x-2 relative">
        <button class="btn-default" on:click={() => { createThread(backendUrl, threadSettings)}}>Create</button>
        <button class="btn-default" on:click={closeSettings}>Close</button>

        {#if $activeThread }
          <button class="btn-default absolute right-0" title="{$activeThread.thread_id}" on:click={() => { if ($activeThread) deleteThread(backendUrl, $activeThread.thread_id); }}>Delete</button>
        {/if}
      </div>
    </div>
  </div>
