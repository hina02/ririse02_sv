<script lang="ts">
  ...
  import { Threads, Tags, activeThread, showSettings } from './Store.ts';
...
<div class="mb-8 py-2 h-16 bg-white dark:bg-blue-800">
  <div class="grid grid-cols-10 justify-between items-center space-y-4">
    ...
    <div class="col-span-5 text-center">
      <h2 class="text-2xl font-bold text-blue-900 dark:text-blue-100">{$activeThread ? $activeThread.name : 'Thread'}</h2>
      <p class="text-blue-500 dark:text-blue-400 overflow-hidden line-clamp-2">{$activeThread ? $activeThread.description : 'description'}</p>
    </div>
    <!-- Select Thread -->
    <div class="col-span-4 flex justify-center space-x-2">
      <!-- Filter Tag -->
      <select bind:value={selectedTag} on:change={filterByTag}
      ...
      <!-- Select Thread -->
      <select bind:value={selectedThread} on:change={selectThread}
        class="flex h-10 items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 w-28 overflow-x-hidden">
        <option disabled selected value> -- Thread -- </option>
        {#if selectedTag}
          {#each filteredThreads as thread}
            <option value={thread.name} selected={$activeThread && $activeThread.name === thread.name}>{thread.name}</option>
          {/each}
        {:else}
