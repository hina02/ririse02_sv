...
export async function updateThreads(backendUrl: string) {
  let threads: ThreadSchemaType[] = await fetch(`${backendUrl}/assistant/get_threads`).then((r) =>
    r.json()
  )
...
export async function deleteThread(backendUrl: string, thread_id: string) {
  const response = await fetch(`${backendUrl}/assistant/delete_thread/${thread_id}`, {
    method: 'DELETE',
  })
  if (response.ok) {
    const responseText = response.text()
