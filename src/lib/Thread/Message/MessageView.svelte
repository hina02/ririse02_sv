<script lang="ts">
  ...
  import { getMessages, updateMessages } from './getMessage'
...
<div class="relative md:space-y-4 bg-blue-100 h-full w-full rounded-md overflow-y-auto">
  ...
  {:else if activeMessages.length > 0}
    {#each activeMessages as message (message.id)}
      <div class="grid md:grid-cols-10 grid-cols-1 pl-4 pt-2">
        ...
        <div class="md:col-span-8 col-span-1 pr-2 text-ellipsis overflow-hidden">
          {message.content}
        </div>
  ...
  {#if $activeThread}
    <div class="sticky bottom-2 right-4 z-10 flex justify-end">
      <button
        class="sticky bottom-2 right-4 z-10 material-icons base_icon text-gray-400"
        title="Update Message"
        on:click={updateMessages(backendUrl)}>refresh</button
      >
    </div>
  {/if}
</div>
