...
import { activeThread } from '../../../lib/Thread/Thread/Store.js';
import { Messages } from './MessageStore.ts';
...
export async function updateMessages(backendUrl: string) {
	const thread_id = get(activeThread).thread_id; // ストアからthread_idを取得
	const messages = await fetch(`${backendUrl}/openai_api/get_messages/${thread_id}`).then((r) => r.json());
...
export async function createMessage(backendUrl: string, content: string) {
	...
	const response = await fetch(`${backendUrl}/openai_api/create_message/${selectedThread}?content=${encodeURIComponent(content)}`, {
		method: 'POST'
	});
...
export async function createMessageAndRun(backendUrl: string, content: string, assistant_id: string) {
	if (assistant_id === undefined) {
		AssistantIDErrorMessage
		return;
	}
	...
	const response = await fetch(
		`${backendUrl}/openai_api/create_message_and_run/${selectedThread}/${assistant_id}?content=${encodeURIComponent(content)}`, {
			method: 'POST'
		});
	await handleResponse(response, 'メッセージの作成、Runが正常に実行されました', 'エラーが発生しました');
}
