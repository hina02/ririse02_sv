<script lang="ts">
  ...
  import { getFiles, uploadFiles, deleteFile } from './getFile'
    ...
    <details class="w-full">
            ...
            <tbody class="w-full break-all text-[0.5rem]">
              {#each $Files as file}
                <tr class="border-b-2 border-blue-200">
                  <td class="pr-1" on:click={() => (selectedFileId = file.file_id)}
                    >{file.file_id}</td
                  >
                  <td>{file.filename}</td>
      ...
      <div class="mt-4 space-x-2 relative">
        <button
          ...
          title="Upload Files"
          on:click={() => {
            uploadFiles(backendUrl, files.accepted)
            resetFiles()
          }}>Upload Files</button
        >
        {#if selectedFileId}
          <button
            ...
            on:click={() => {
              if (selectedFileId) {
                deleteFile(backendUrl, selectedFileId)
                selectedFileId = null
              }
            }}>Delete</button
          >
